...
public class JwtToken {
    ...
    private static final DateTimeFormatter ID_FORMAT =
            DateTimeFormatter.ofPattern("MMddkkmmss").withZone(ZoneId.systemDefault());
    private static final ConcurrentLinkedQueue<JwtRsaDigitalSignatureKey> keysQueue =
            new ConcurrentLinkedQueue<JwtRsaDigitalSignatureKey>();
     ...
     */
    public static synchronized String getToken(Object context, GlobalAkeylessConfiguration globalConfig) {
    ...
    public static synchronized String getToken(
            String pluginAction, Object context, GlobalAkeylessConfiguration globalConfig) {
     ...
     * @return JWTToken
     */
    public static synchronized JwtToken getUnsignedToken(
            String pluginAction, Object context, GlobalAkeylessConfiguration globalConfig) {
        if (context == null) {
        ...
        if (contextObject instanceof AbstractItem) {
            ...
            if (!isEnabled) {
                ...
                // Add identity field
                List<String> identityFields = Arrays.asList(
                        globalConfig.getIdentityFormatFieldsFromToken().split(","));
                ...
                for (String identityField : identityFields) {
                    String identityFieldValue =
                            jwtToken.claim.has(identityField) ? jwtToken.claim.getString(identityField) : "";
                    identityValues.add(identityFieldValue);
                    LOGGER.log(
                            Level.FINE,
                            "getUnsignedToken() *** processed identity field:" + identityField + " and value:"
                                    + identityFieldValue);
                }
                ...
                List<String> identityFields = Arrays.asList(
                        globalConfig.getSelectIdentityFormatToken().split("[-,+,|,:,.]"));
                List<String> identityValues = new ArrayList<>(identityFields.size());
        ...
        LOGGER.log(Level.FINEST, String.format("Claim : %s", jwtToken.claim.toString(4)));
        return jwtToken;
    }
    ...
    protected static synchronized JwtRsaDigitalSignatureKey getCurrentSigningKey(JwtToken jwtToken) {
        ...
        long currentTime = System.currentTimeMillis() / 1000;
        long maxKeyTimeInSec = GlobalAkeylessConfiguration.get().getKeyLifetimeInMinutes() * 60;
     ...
     */
    protected static synchronized JSONObject getJWTKeys() {
                    ...
                    if (currentTime - key.getCreationTime() < maxKeyTimeInSec) {
                        ...
                        jwk.put(
                                ...
                                Base64.getUrlEncoder()
                                        .withoutPadding()
                                        .encodeToString(
                                                key.getPublicKey().getModulus().toByteArray()));
                        jwk.put(
                                "e",
                                Base64.getUrlEncoder()
                                        .withoutPadding()
                                        .encodeToString(key.getPublicKey()
                                                .getPublicExponent()
                                                .toByteArray()));
                        keys.put(jwk);
